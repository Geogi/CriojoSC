...
class SuccessorSpec extends Specification with RuleImplicits {
  def is =
    ...
    "Specification of the Successor[Int] constructor." ^
      ...
      "Create a pattern with the companion object's '?' method..." ! {
        Successor ? (1) must bePatternInt
      } ^
      "... or with the shortcut 'S?'." ! {
        S ? (1) must bePatternInt
      } ^
