...
/** A symbolic representation of an [[fr.emn.criojomin.Atom]], as used in rule premises and "where" guards.<br />
  * The engine matches it against the actual instances in [[fr.emn.criojomin.EntitySymbol]] to produce a
  * [[fr.emn.criojomin.Valuation]], used by most [[fr.emn.criojomin.Guard]]s and
  * [[fr.emn.criojomin.Rule]] conclusions.
  *
  * Note that while [[fr.emn.criojomin.AtomPattern]] is not typed itself, it is created by
  * an AtomN ([[fr.emn.criojomin.Atom0]], [[fr.emn.criojomin.Atom1]], etc) that
  ...
  * @param patterns The list of [[fr.emn.criojomin.Pattern]]s used by the matching process.
  */
class AtomPattern(val symbol: Atom, val patterns: List[Pattern[_]]) extends Term

/** An instance of an [[fr.emn.criojomin.Atom]] in a given state (a list of an unbound type).<br />
  * It may exist in [[fr.emn.criojomin.EntitySymbol]] or be the product of a [[fr.emn.criojomin.Rule]]
  * (listed in its conclusion).
  *
  * Note that while [[fr.emn.criojomin.AtomInstance]] is not typed itself, it is created by
  * an `AtomN` ([[fr.emn.criojomin.Atom0]], [[fr.emn.criojomin.Atom1]], etc) that
  * is typed, ensuring a consistent state.<br />
  * This is why instances of this class should not be created manually, but using `Atom.apply` (arity > 0) or
  * `Atom0.unary_!`.
...
/** An atom identifier, which can create [[fr.emn.criojomin.AtomPattern]]s and
  * [[fr.emn.criojomin.AtomInstance]]s.<br />
  * Its implementations are arity dependent: [[fr.emn.criojomin.Atom0]] to (currently)
  * [[fr.emn.criojomin.Atom2]] (will expand to Atom22, to match `scala.Product`).
  *
...
/** An identifier for a nullary atom. See [[fr.emn.criojomin.Atom]] for more details. */
class Atom0 extends Atom {
  /** Creates an [[fr.emn.criojomin.AtomPattern]] of this atom */
  def unary_~ = new AtomPattern(this, Nil)

  /** Creates an [[fr.emn.criojomin.AtomInstance]] of this atom */
  def unary_! = new AtomInstance(this, Nil)
}

/** An identifier for an unary atom. See [[fr.emn.criojomin.Atom]] for more details. */
class Atom1[T1] extends Atom {
  /** Creates an [[fr.emn.criojomin.AtomPattern]] of this atom with pattern `p1`. */
  def apply(p1: Pattern[T1]) = new AtomPattern(this, List(p1))

  /** Creates an [[fr.emn.criojomin.AtomInstance]] of this atom with value `v1`. */
  def apply(v1: T1) = new AtomInstance(this, List(v1))
}

/** An identifier for a binary atom. See [[fr.emn.criojomin.Atom]] for more details. */
class Atom2[T1, T2] extends Atom {
  /** Creates an [[fr.emn.criojomin.AtomPattern]] of this atom with patterns `p1`, `p2`. */
  def apply(p1: Pattern[T1], p2: Pattern[T2]) = new AtomPattern(this, List(p1, p2))

  /** Creates an [[fr.emn.criojomin.AtomInstance]] of this atom with values `v1`, `v2`. */
  def apply(v1: T1, v2: T2) = new AtomInstance(this, List(v1, v2))
}

/** This object provides utilities for creating [[fr.emn.criojomin.Atom]]s.
  *
  * @example {{{
  ...
  */
object Atom {
  /** Creates a nullary atom ([[fr.emn.criojomin.Atom0]]) */
  def apply() = new Atom0

  /** Creates an unary atom ([[fr.emn.criojomin.Atom1]]) with signature `T1` */
  def apply[T1] = new Atom1[T1]

  /** Creates a binary atom ([[fr.emn.criojomin.Atom2]]) with signature `T1`, `T2` */
