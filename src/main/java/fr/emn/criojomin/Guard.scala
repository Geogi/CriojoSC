...
/** Guards allow conditional triggering of [[fr.emn.criojomin.Rule]]s. !CURRENTLY A STUB!
  *
  * Their most useful feature is that they permit introspection of the [[fr.emn.criojomin.EntitySymbol]], allowing
  * tests that would not be possible only with the premise.<br />
  * For instance, `Abs(~A)` test if no [[fr.emn.criojomin.AtomInstance]] of A is present in the solution.
  */
trait Guard {
...
/** && guard (using a [[fr.emn.criojomin.Guard]] method is probably preferable) !CURRENTLY A STUB! */
...
/** `true` if the premise match and, given the updated [[fr.emn.criojomin.Valuation]], the sub-guard is `true` !CURRENTLY A STUB! */
...
/** The parent trait of all guards that require special handling by the [[fr.emn.criojomin.EntitySymbol]] (inspectors). !CURRENTLY A STUB! */
